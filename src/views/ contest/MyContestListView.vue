  ...
  <div id="MyContestListView">
    ...
    >
      ...
      <template #title="{ record }">
        <span @click="toContesetPage(record)" class="question_title">{{
          record.title
        }}</span>
      </template>
      <template #type="{ record }">
        <a-tag v-if="record.type === 0" color="green" style="font-size: 15px">
          <span> 基础 </span>
        </a-tag>
        <a-tag
...
const loadData = async () => {
  const res = await ContestControllerService.listMyContestVoByPageUsingPost(
    searchParams.value
  );
  if (res.code === 0) {
 ...
 */
const toContesetPage = (contestVO: ContestVO) => {
  router.push({
